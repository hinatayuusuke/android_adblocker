...
## 7.Compatibility / Fallback policy (token-efficient)

- 互換性対応（旧API/旧形式/旧バージョンのサポート）は、要件や指示で明示された場合のみ実装する。
- フォールバック（代替実装・自動切替・多重分岐・サイレントリトライ）は、要件で明示された場合のみ追加する。
- 不明な場合は、失敗時は明示的に失敗させる（fail fast）前提で実装する。
- 将来互換のためだけの抽象化・拡張ポイント・予防的分岐は追加しない。
- 例外: 互換性/フォールバックが必要な場合は、対象（例: version range / runtime）と理由をコメントで明記する。